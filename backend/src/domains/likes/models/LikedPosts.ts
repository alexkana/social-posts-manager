...
likedPostsSchema.statics.hasLiked = async function (
  userId: string,
  postId: string,
): Promise<boolean> {
...
likedPostsSchema.statics.findLikedByUser = function (
  userId: string,
): Promise<ILikedPost[]> {
...
likedPostsSchema.statics.findLikersOfPost = function (
  postId: string,
): Promise<ILikedPost[]> {
...
export default mongoose.model<ILikedPost, LikedPostModel>(
  "LikedPosts",
  likedPostsSchema,
);
